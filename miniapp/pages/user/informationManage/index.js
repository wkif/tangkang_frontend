...
Page({
  ...
  PickerChangeheight(e) {
    ...
    if (app.globalData.speedFlag) {
      app.$Text2Speech(
        `身高已经更改为${this.data.heightPicker[e.detail.value]}厘米`
      );
    }
    this.setData({
  ...
  PickerChangegender(e) {
    if (app.globalData.speedFlag) {
      app.$Text2Speech(
        `性别已经更改为${this.data.heightPicker[e.detail.value]}`
      );
    }
    this.setData({
  ...
  PickerChangeweight(e) {
    ...
    if (app.globalData.speedFlag) {
      app.$Text2Speech(
        `体重已经更改为${this.data.weightPicker[e.detail.value]}公斤`
      );
    }
    this.setData({
  ...
  PickerChangebloodType(e) {
    ...
    if (app.globalData.speedFlag) {
      app.$Text2Speech(
        `血型已经更改为${this.data.bloodTypePicker[e.detail.value]}`
      );
    }
    this.setData({
  ...
  PickerChangewaistline(e) {
    ...
    if (app.globalData.speedFlag) {
      app.$Text2Speech(
        `腰围已经更改为${this.data.waistlinePicker[e.detail.value]}厘米`
      );
    }
    this.setData({
  ...
  async onChooseAvatar(e) {
    ...
    // let res = await this.base64(avatarUrl,'png');
    ...
    let avatarData = await app.$api.uploadImg(data);
    console.log("avatarData", avatarData);
    if (avatarData.code == 0) {
      this.setData({
        "userInfo.avatar": avatarData.data.imgUrl,
      });
    } else {
      this.setData({
