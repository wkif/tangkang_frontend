...
const config = require('../../config'),
        ...
        if (config.highlight.length && hljs) {
            result.highlight = (code, lang, callback) => {
                let lineLen = code.split(/\r|\n/gi).length,
                    result = hljs.highlightAuto(code).value; // 代码块多换行的问题
    ...
    md = require('./markdown')(mdOption); // 应用Markdown解析扩展，包括自定义组件（['sub','sup','ins','mark','emoji','todo','latex','yuml','echarts']）
...
md.use(require('./plugins/yuml')); // 定义emoji渲染规则
