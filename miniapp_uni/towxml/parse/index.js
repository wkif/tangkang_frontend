const parse2 = require('./parse2/index'),
    ...
    initObj = (obj, option) => {
        const result = {
            ...
            base = option.base; // 主题保存到全局

        global._theme = result.theme; // 事件添加到全局中，各个组件在触发事件时会从全局调用
        ...
        (eachFn = (arr, obj, _e, isRichTextContent) => {
            ...
            arr.forEach((item) => {
                ...
                if (item.type === 'text') {
                    ...
                    attrs.class = attrs.class ? `h2w__${item.name} ${attrs.class}` : `h2w__${item.name}`; // 处理资源相对路径
